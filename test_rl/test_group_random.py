...
import numpy as np
from env_random import ConstraintSimplificationEnv_test
...
def test_group():
    if not os.path.exists('info_dict_random.txt'):
        # 文件不存在时，创建文件
        info_dict = {}
        with open('info_dict_random.txt', 'w') as file:
            json.dump(info_dict, file, indent=4)
        print(f"文件 info_dict_random.txt 已创建。")
    else:
        info_dict = load_dictionary('info_dict_random.txt')
        print(f"文件已存在。")
        ...
        if list1[0] == "sat":
            ...
            # if '/who/who86404' in key:
                ...
                if file_path not in info_dict.keys():
                    ...
                    env = ConstraintSimplificationEnv_test(assertions, len(variables), len(variables), smtlib_str, file_path)
                    env.reset()
                    number_of_steps = 500
