...
class ConstraintSimplificationEnv_test(Environment):
    ...
    def __init__(self,z3ast, num_variables, num_constants, smtlib_str, file_path):
        self.step_count = 0
        ...
        self.variables_2 = self.variables.copy()
    ...
    def step(self):
        ...
        self.step_count += 1
        try:
            if len(self.variables_2) > 1:
                random_index = random.randint(0, len(self.variables_2) - 1)
            else:
                random_index = 0
            # 使用pop方法删除并返回列表中随机索引位置的元素
            variable_pred = self.variables_2.pop(random_index)
            ...
            if variable_pred not in self.used_variables:
                ...
                # 如果选择了最后一个数，随机选择一个值
                selected_int = int(random.choice(list(dict_value.values())))
            ...
            print(e)
            print(len(self.variables_2))
            reward = 0
        # if self.step_count > 500:
        #     self.finish = True
        if len(self.variables_2) == 0:
            self.reset()
    @staticmethod
