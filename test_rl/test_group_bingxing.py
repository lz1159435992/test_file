...
import numpy as np
from env_bingxing import ConstraintSimplificationEnv_test
...
def test_group():
    ...
    if not os.path.exists('info_dict_bingxing.txt'):
        # 文件不存在时，创建文件
        info_dict = {}
        ...
        info_dict = load_dictionary('info_dict_bingxing.txt')
        print(f"文件已存在。")
            ...
            # if '/who/who86404' in key:
                ...
                if file_path not in info_dict.keys():
                    ...
                    if env.step_count > 1000:
                        ...
                        result_list.append(env.solve_v)
                    # if env.threads:
                    #     for t in env.threads:
                    #         if t.is_alive():
                    #             t._stop()
                    # if env.process is not None:
                    #     if env.process.is_alive():
                    #         # env.process.terminate()
                    #         pid = env.process.pid  # 获取进程的PID
                    #         os.kill(pid, signal.SIGTERM)  # 发送SIGTERM信号
