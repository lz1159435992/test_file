...
from pearl.api import ActionSpace
...
def test_group():
    if not os.path.exists('file_time.txt'):
        ...
        #     json.dump(file_time, file, indent=4)
        # info_dict = {}
    if not os.path.exists('info_dict.txt'):
        # 文件不存在时，创建文件
        info_dict = {}
        with open('info_dict.txt', 'w') as file:
            json.dump(info_dict, file, indent=4)
        print(f"文件 info_dict.txt 已创建。")
    else:
    ...
    policy_learner = SoftActorCritic(
        state_dim=768,
        action_space=DiscreteActionSpace([torch.tensor(0), torch.tensor(-10000)]),
        actor_hidden_dims=[768, 512, 128],
        critic_hidden_dims=[768, 512, 128])

    for key, value in result_dict.items():
        list1 = json.loads(value)
        if list1[0] == "sat" and list1[1] > 20:
        # if '143541' in key:
            print(key, value)
            ...
            if file_path not in info_dict.keys():
                ...
                env = ConstraintSimplificationEnv_test(embedder, assertions, len(variables), len(variables),
                                                       smtlib_str,
                                                       file_path)
                ...
                policy_learner.action_space = action_space
                policy_learner.action_representation_module = action_representation_module
                agent = PearlAgent(
                ...
                if env.step_count > 500:
                    ...
                    result_list.append(env.slove_time)
